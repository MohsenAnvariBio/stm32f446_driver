...
void USART_SendData(USART_Handle_t *pUSARTHandle, uint8_t *pTxBuffer, uint32_t Length)
{
    ...
    for(uint32_t i = 0 ; i < Length; i++)
    {
        ...
        if(pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS)
        {
            ...
            if(pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE)
            {
            	...
            	// if parity is disabled, so then 9 bits will be of user data. it's like transferring 2 bytes.
            	// So, because to hold 9 bits we need 2 bytes that's why the buffer address is incremented by 2
                pTxBuffer++;
                pTxBuffer++;
            }
            else
    ...
    // that actually ensures that all the bits are successfully transferred from the USART to the external world
...
void USART_IRQHandling(USART_Handle_t *pUSARTHandle)
{
    ...
    //Not discussed multibuffer communication in this course. please refer to the RM
