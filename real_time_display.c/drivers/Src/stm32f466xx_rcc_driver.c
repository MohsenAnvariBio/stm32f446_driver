...
uint32_t RCC_GetPCLK1Value(void)
{
    ...
    // Determine system clock based on source
    if(clksrc == 0) // HSI
    {
        SystemClk = 16000000; // Assuming HSI is 16 MHz
    }
    ...
    /* AHBP */
    // Read AHB prescaler value and calculate division factor
    temp = ((RCC->CFGR >> 4) & 0xF);
    ...
    /* APB1 */
    // Read APB1 prescaler value and calculate division factor
...
uint32_t RCC_GetPCLK2Value(void)
{
    ...
    if(temp < 8) // if this value is less than 8 then system clock is not divided.
    {
        ahbp = 1;
    }
    else
