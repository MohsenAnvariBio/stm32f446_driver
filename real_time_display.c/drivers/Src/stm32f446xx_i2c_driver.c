...
void I2C_Init(I2C_Handle_t *pI2CHandle)
{
    ...
    tempreg = 0;
    tempreg|= RCC_GetPCLK1Value() / 1000000U; // because we only want the number 16, so not the entire value.
    pI2CHandle->pI2Cx->CR2 = (tempreg & 0x3F);

    /* Programming device own address */
    tempreg = 0;
    tempreg |= pI2CHandle->I2C_Config.I2C_DeviceAddress << 1; // because, we should not occupy the ADD0 bit,
//    tempreg |= (1 << I2C_OAR1_ADDMODE); // if you are using 10 bit, set ADD MODE bit to 1.
    tempreg |= (1 << 14); // Bit 14 Should always be kept at 1 by software.
    pI2CHandle->pI2Cx->OAR1 = tempreg;

    /* CCR calculations */
    uint16_t ccr_value = 0; // uint16_t type: only 12 bits are valid. In the reference manual CCR is 12 bits.
    tempreg = 0;
    if(pI2CHandle->I2C_Config.I2C_SCLSpeed <= I2C_SCL_SPEED_SM)
    {
        /* Standard mode */
        ccr_value = (RCC_GetPCLK1Value() / (2 * pI2CHandle->I2C_Config.I2C_SCLSpeed)); // Formula of standard mode
 ...
 * @return      - None
 *
 * @Note        - non-blocking API of master send data.
 *
 *****************************************************************/
...
uint8_t I2C_MasterSendDataInterruptMode(I2C_Handle_t *pI2CHandle, uint8_t *pTxBuffer, uint32_t Length, uint8_t SlaveAddr, uint8_t Sr)
{
    ...
    {
        // save all the details into the header
        ...
        /* Generating start condition */
        I2C_GenerateStartCondition(pI2CHandle->pI2Cx); // Start Bit will be set and that will cause an interrupt

        //after that sending of other data like address phase and other things will be taken care by the interrupt service routine
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
    ...
    // TXE interrupt will happen when both these control bits ITEVTEN + ITBUFEN are enabled.
...
static void I2C_ExecuteAddressPhaseWrite(I2C_RegDef_t *pI2Cx, uint8_t SlaveAddr)
{
    SlaveAddr = SlaveAddr << 1; // shift the slave address by one bit making space for the read/write bit.
